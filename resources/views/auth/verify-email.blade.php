<x-auth-layout>
    <div class="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl my-8">
        <div class="p-8">
            ...
            <!-- Message -->
            <div class="mb-6 text-sm text-gray-700 bg-pink-50 p-4 rounded-lg border-l-4 border-pink-300">
            ...
            <div class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
                <form method="POST" action="{{ route('verification.send') }}" class="w-full sm:w-auto">
                    @csrf
                    <button type="submit" class="w-full px-6 py-3 bg-pink-500 hover:bg-pink-600 text-white font-medium rounded-lg text-sm transition duration-300 ease-in-out flex items-center justify-center">
                ...
                <form method="POST" action="{{ route('logout') }}" class="w-full sm:w-auto">
                    @csrf
                    <button type="submit" class="w-full px-6 py-3 border border-pink-300 text-pink-600 hover:bg-pink-50 font-medium rounded-lg text-sm transition duration-300 ease-in-out flex items-center justify-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <div class="mt-8 border-t border-pink-100 pt-4">
                <p class="text-xs text-center text-pink-400">
